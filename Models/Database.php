...
class Database extends PDO
{
    ...
    public function __construct($username, $password, $host, $database)
    {
        try {
            ...
            // Catch any failure to connect to the database and echo the error message
            echo $e->getMessage();
        }
    }
    ...
    public static function getInstance()
    {
        ...
        $password = "abc123456*!";
        $host = "localhost:3306";
        $database = "parcelbuddy";

        if (self::$_dbInstance === null) {
            // Checks if the PDO instance exists
            self::$_dbInstance = new self(
                $username,
                $password,
                $host,
                $database,
            ); // Creates a new instance if not, sending in connection info
        }
